...
const Books = () => {
  ...
  const { openBookDetailModal } = useBookDetailModalContext();
  const { openAddBookModal } = useAddBookModalContext();
  ...
  const fetchBooks = useCallback(async () => {
    try {
      ...
      const url = `${API_ENDPOINTS.BOOKS}?page=${page}&size=${pageSize}`;
      ...
      const books: Book[] = data.content.map((book) => ({
        id: book.id,
        title: book.title,
        category: book.category,
        publishedYear: book.publishedYear,
        author: book.author,
        image: "https://play.google.com/books/publisher/content/images/frontcover/GTGPEQAAQBAJ?fife=w256-h256",
      }));

      setBooks(books);
      setTotalPages(data.totalPages);
      setTotalBooks(data.content.length);
  ...
  useEffect(() => {
    ...
    window.addEventListener("books:refresh", onRefresh);
    return () => window.removeEventListener("books:refresh", onRefresh);
  }, [fetchBooks]);
        ...
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="flex gap-7.5">
            ...
            <div className="w-full">
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <p>
                      Showing <span className="text-dark">{totalBooks} of {totalBooks}</span>{" "}
                      Books
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5">
                    <button onClick={() => openAddBookModal()} className="rounded bg-blue px-3 py-1 text-white mr-3">
                      Add New Book
                    </button>
                    <button
                      onClick={() => setBookStyle("grid")}
                      aria-label="button for book grid tab"
                      className={`${
                        bookStyle === "grid"
                    ...
                    <button
                      onClick={() => setBookStyle("list")}
                      aria-label="button for book list tab"
                      className={`${
                        bookStyle === "list"
                          ? "bg-blue border-blue text-white"
                          : "text-dark bg-gray-1 border-gray-3"
              ...
              {/* Books Grid Tab Content Start */}
              {loading ? (
                <div className="flex justify-center py-12">
                  <p className="text-gray-5">Loading books...</p>
                </div>
              ) : error ? (
                <div className="flex justify-center py-12">
                ...
                <div className="flex justify-center py-12">
                  <p className="text-gray-5">No books available.</p>
                </div>
              ) : (
                <div
                  className={`${
                  ...
                  {books.map((item, key) =>
                    bookStyle === "grid" ? (
                      <SingleGridItem
                        item={item}
                        key={key}
                        onViewBook={openBookDetailModal}
                      />
                    ) : (
                      <SingleListItem
                        item={item}
              ...
              {/* Books Grid Tab Content End */}

              {/* Books Pagination Start */}
